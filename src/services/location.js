...
const { COUNTRY_STATE_CITY_API_ERROR, COUNTRY_CODE_REQUIRED, STATE_CODE_REQUIRED } = require('~/consts/errors')
...
const getCountryData = ({ name, iso2 }) => ({
  name,
  iso2
})
...
const locationService = {
  ...
  getCities: async (countryCode, stateCode) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/countries/${countryCode}/states/${stateCode}/cities`, {
        method: 'GET',
        headers,
        signal: AbortSignal.timeout(timeout)
      })
      ...
      if (error.code === COUNTRY_CODE_REQUIRED.code || error.code === STATE_CODE_REQUIRED.code) {
        throw error
      }
